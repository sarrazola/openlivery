...
import { AlertCircle, Check, CheckCheck, Clock, CircleHelp } from "lucide-react";
...
export function DeliveryTicks({ status, error }: { status?: string | null; error?: string | null }) {
  ...
  if (status === "pending") return <span className="ticks" title={t("social.deliveryPending")}><Clock size={12} /></span>;
  if (status !== "sent" && status !== "delivered" && status !== "read") return <span className="ticks" title={t("social.deliveryUnknown")}><CircleHelp size={12} /></span>;
