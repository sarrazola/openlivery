...
import { BarChart3, Bot, Building2, CheckCircle2, Clock, Contact as ContactIcon, FileText, Images, Inbox, LoaderCircle, LogOut, MessageSquareText, Reply, Search, Send, ShieldCheck, SmilePlus, UserRound, Users, X } from "lucide-react";
...
function PortalInbox({ slug, portal, session, logout }: { slug: string; portal: PortalPublic; session: Session; logout: () => void }) {
  ...
  const visibleItems = useMemo(() => items.filter((item) => (item.status === "resolved") === (status === "resolved") && (!channelFilter || item.channel === channelFilter)), [items, status, channelFilter]);
