...
const lineState = (line: WhatsAppChannel): LineState =>
  line.status === "connected" ? "connected" : ["qr", "connecting", "reconnecting"].includes(line.status) ? "pending" : "disconnected";

/** A client's WhatsApp QR lines. One is selected at a time and the panels
 * below configure that one; "add another number" starts a new line. */
export default function WhatsAppChannelPage() {
  ...
  return <div className="page wa-page">
    ...
    <LineSwitcher lines={lines.map((line) => ({ id: line.id, name: nameOf(line), state: lineState(line) }))} selectedId={selectedId} adding={adding} addLabel={t("clients.whatsapp.addLine")} newLabel={t("clients.whatsapp.newLine")} onSelect={(lineId) => show(lines.find((line) => line.id === lineId) ?? null, client)} onAdd={() => { setAdding(true); setSelectedId(null); setAgentId(client.agents[0]?.id || ""); setLabel(""); setError(""); }} />
