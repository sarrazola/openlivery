  ...
  return (
    <div className="page">
      ...
      <section className="metrics-grid">
        <article className="metric-card"><span className="metric-icon blue"><Building2 size={20} /></span><div><small>{t("home.metrics.clients")}</small><strong>{loadedCore ? data?.clients ?? "—" : <Skeleton className="sk-line lg" style={{ width: 52 }} />}</strong><p>{t("home.metrics.clientsActive", { count: data?.active_clients ?? 0 })}</p></div></article>
        <article className="metric-card"><span className="metric-icon violet"><Bot size={20} /></span><div><small>{t("home.metrics.agents")}</small><strong>{loadedCore ? agents.length || data?.agents || "—" : <Skeleton className="sk-line lg" style={{ width: 52 }} />}</strong><p>{t("home.metrics.agentsActive", { count: agents.filter((item) => item.is_active).length })}</p></div></article>
        <article className="metric-card"><span className="metric-icon green"><MessageSquareText size={20} /></span><div><small>{t("home.metrics.conversations")}</small><strong>{loadedCore ? conversations.length : <Skeleton className="sk-line lg" style={{ width: 52 }} />}</strong><p>{t("home.metrics.conversationsCaption")}</p></div></article>
        <article className="metric-card"><span className="metric-icon amber"><Radio size={20} /></span><div><small>{t("home.metrics.channels")}</small><strong>{loadedCore ? data?.channels ?? "—" : <Skeleton className="sk-line lg" style={{ width: 52 }} />}</strong><p>{t("home.metrics.channelsConnected", { count: data?.connected_channels ?? 0 })}</p></div></article>
      </section>
